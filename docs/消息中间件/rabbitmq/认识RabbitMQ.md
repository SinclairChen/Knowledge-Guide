...
![image-20210626133428330](image/image-20210626133428330.png)
...
生产者，就是投递消息的一方。生产者创建消息，然后将消息发布到RabbitMQ中。



消息一般包含两个部分：

- 消息体（payload）：一般是一个带有业务逻辑结构的数据，比如JSON字符串
- 附加信息：用来表述这一条消息，比如目标交换机的名称，路由键和一些自定义的属性等。
...
1. 生产者连接到RabbitMQ Broker，建立一个链接（Connection），开启一个信道（channel）
2. 生产者声明一个交换机，并设置相关属性，比如：交换机类型、是否持久化等
3. 生产者声明一个队列并设置相关属性，比如：是否排他，是否持久化，是否自动删除等
4. 生产者通过路由键将交换机和队列绑定起来
5. 生产者发送消息至RabbitMQ Broker，其中包含路由键、交换机等信息
6. 相应的交换机根据接受到的路由键找到相匹配的队列
7. 如果找到对应的队列，则将消息路由到队列中
8. 如果没有找到对应的队列，则根据生产者配置的属性选择丢弃还是回退给生产者
9. 关闭信道和连接
...
2. 消费者向RabbitMQ请求消费相应的队列，可能会设置相应的回调函数，以及做一些准备工作
3. 等待RabbitMQ Borker回应并投递相应队列中的消息
4. 消费者接受到消息，并确认（回复ACK）
5. RabbitMQ 的相应队列中删除这条已经被确定的消息
6. 关闭信道和连接
